...
class TestCreateBotToken:
    ...
    def test_roles_embedded(self, keypair):
        ...
        token = create_bot_token(
            private, "my-bot", ["admin", "readonly"], "admin@example.com", 30
        )
        claims = _decode(token, public)
        assert set(claims["roles"]) == {"admin", "readonly"}

    def test_issuer_and_audience(self, keypair):
        private, public = keypair
        token = create_bot_token(private, "my-bot", [], "admin@example.com", 30)
        claims = _decode(token, public)
    ...
    def test_jti_is_unique(self, keypair):
        private, _ = keypair
        t1 = create_bot_token(private, "my-bot", [], "admin@example.com", 30)
        t2 = create_bot_token(private, "my-bot", [], "admin@example.com", 30)
    ...
    def test_iat_close_to_now(self, keypair):
        private, public = keypair
